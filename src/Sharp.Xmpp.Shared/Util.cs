...
namespace Sharp.Xmpp
{
    ...
    internal static class Util
    {
        ...
        internal static Exception ExceptionFromError(Iq errorIq, string message = null)
        {
            ...
            return ExceptionFromError(errorIq.Data.GetNodeName("error"), message);
        }
        ...
        /// could be created from the specified Iq stanza, or an exception of type
        /// XmppException denoting an unrecoverable error.</returns>
        internal static Exception ExceptionFromError(XElement error, string message = null)
        {
            try
            {
        ...
        /// <summary>
        /// Take and remove item from collection
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="collection"></param>
        /// <returns></returns>
        internal static bool TakeOut<K, T>(this Dictionary<K, T> collection, K key, out T item)
        {
            if (collection.Count > 0 && collection.ContainsKey(key))
            {
                item = collection[key];
        ...
        /// <param name="collection"></param>
        /// <returns></returns>
        internal static T TakeOut<T>(this List<T> collection, int index = 0)
        {
