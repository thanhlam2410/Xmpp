...
namespace Sharp.Xmpp.Core.Sasl.Mechanisms
{
    ...
    internal class SaslScramSha1 : SaslMechanism
    {
        ...
        {
            Dictionary<string, string> nv = ParseServerFirstMessage(challenge);
            ...
            string clientFirstBare = "n=" + SaslPrep(Username) + ",r=" + Cnonce,
                serverFirstMessage = Encoding.UTF8.GetString(challenge, 0, challenge.Length),
                withoutProof = "c=" +
            ...
            SaltedPassword = Hi(Password, salt, (uint)iterationCount);
        ...
        {
            string s = Encoding.UTF8.GetString(challenge, 0, challenge.Length);
        ...
        private Dictionary<string, string> ParseServerFirstMessage(byte[] challenge)
        {
            ...
            string message = Encoding.UTF8.GetString(challenge, 0, challenge.Length);
        ...
        private byte[] Hi(string password, string salt, uint count)
        {
            // The salt is sent by the server as a base64-encoded string.
            byte[] saltBytes = Convert.FromBase64String(salt);
            return Crypto.RFC2898DerivesBytes(password, saltBytes, count);
        }
        ...
        {
            return Crypto.SHA1Managed(data);
        }
