...
namespace Sharp.Xmpp.Core.Sasl.Mechanisms
{
    ...
    internal class SaslDigestMd5 : SaslMechanism
    {
        ...
        private byte[] ComputeDigestResponse(byte[] challenge)
        {
            ...
            string decoded = Encoding.UTF8.GetString(challenge, 0, challenge.Length);
            Dictionary<string, string> fields = ParseDigestChallenge(decoded);
            ...
            return Encoding.UTF8.GetBytes(challengeResponse);
        }
            ...
            string cnonce, string digestUri, string username, string password)
        {
            ...
            string A1 = MD5Core.GetHashString(data);
