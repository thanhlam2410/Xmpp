...
namespace Sharp.Xmpp
{
    ...
    public class XmppError
    {
        ...
        public ErrorType Type
        {
            ...
            {
                type = value;
                Data.SetAttributeValue("type", value.ToString().ToLowerInvariant());
            }
        }
        ...
        public string Text
        {
            set
            {
                var text = Data.GetNodeName("text");
                if (text != null)
                {
                    if (value == null)
                    {
                ...
                else
                {
                    if (value != null)
                    {
                        Data.Child(Xml.Element("text", "urn:ietf:params:xml:ns:xmpp-stanzas").Text(value));
                    }
                }
            }
            ...
            get
            {
                var text = Data.GetNodeName("text");
                if (text != null)
                    return string.IsNullOrEmpty(text.Value) ? null : text.Value;
                return null;
            }
        }
        ...
        public XmppError(ErrorType type, ErrorCondition condition, params XElement[] data) :
        ...
        internal XmppError(XElement error)
        {
            error.ThrowIfNull("error");
            // Verify mandatory error type attribute.
            ErrorType type = (ErrorType)Enum.Parse(typeof(ErrorType), error.GetAttributeValue("type"), true);
        ...
        {
            ISet<XElement> set = new HashSet<XElement>();
            // Remove old condition, if any.
            foreach (var v in Enum.GetValues(typeof(ErrorCondition)))
            {
                string s = ErrorConditionToTagName((ErrorCondition)v);
                if (Data.GetNodeName(s) != null)
                    set.Add(Data.GetNodeName(s));
            }
