    ...
    public class Message : Core.Message
    {
        ...
        public MessageType Type
        {
            ...
            set
            {
                ...
                element.SetAttributeValue("type", v);
            }
        }
        ...
        public DateTime Timestamp
        {
            ...
            {
                // Refer to XEP-0203.
                var delay = element.GetNodeName("delay");
                if (delay != null && delay.BaseUri == "urn:xmpp:delay")
                {
                    DateTime result;
                    if (DateTime.TryParse(delay.GetAttributeValue("stamp"), out result))
                        return result;
                }
                return timestamp;
            }
        ...
        public string Thread
        {
            get
            {
                if (element.GetNodeName("thread") != null)
                    return element.GetNodeName("thread").Value;
                return null;
            }

            set
            {
                var e = element.GetNodeName("thread");
                if (e != null)
                {
                    if (value == null)
                        e.Remove();
        ...
        internal Message(Message message)
        {
            ...
            type = ParseType(message.Data.GetAttributeValue("type"));
        ...
        {
            var nodeList = element.Nodes().Where((x) =>
            {
                var item = x as XElement;
                return (item.Name.LocalName == tag);
            });

            foreach (XElement e in nodeList)
            {
                string k = e.GetAttributeValue("xml:lang");
